...
CREATE TABLE shard.kafka_movie_view (
      ...
      ENGINE = Kafka('localhost:9092', 'movie_view', 'frame-viewers-group', 'JSONEachRow');
...
CREATE TABLE IF NOT EXISTS shard.movie_view (
      ...
      viewed_frame Int32,
      timestamp DateTime('Europe/Moscow'))
      Engine=ReplicatedMergeTree('/clickhouse/tables/shard1/movie_view', 'replica_1') PARTITION BY toYYYYMMDD(timestamp) ORDER BY user_id;
...
CREATE TABLE IF NOT EXISTS replica.movie_view (
      ...
      viewed_frame Int32,
      timestamp DateTime('Europe/Moscow'))
      Engine=ReplicatedMergeTree('/clickhouse/tables/shard2/movie_view', 'replica_2') PARTITION BY toYYYYMMDD(timestamp) ORDER BY user_id;
